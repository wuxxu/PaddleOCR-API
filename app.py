...
def _warmup():
    try:
        ...
        buf = "/tmp/warm.png"
        img.save(buf, "PNG")
        _ = ocr.ocr(np.array(img))  # ✅ no cls=True here
...
def clamp_image(pil_img, max_pixels=60_000_000, max_side=4000):
    ...
    sf1 = sqrt(max_pixels / float(w * h))
    sf2 = max_side / float(max(w, h))
    sf = min(sf1, sf2, 1.0)
...
def detect_table_rows(line_words):
    ...
    for ln in lines:
        ...
        gaps = [ln[i]["x"] - ln[i-1]["x"] for i in range(1, len(ln))]
        medGap = median(gaps) or 32
        x_gap = max(28, int(round(medGap * 1.6)))
        cells, buf, last_x = [], "", None
        for w in ln:
            if last_x is not None and (w["x"] - last_x) > x_gap:
                if buf.strip(): cells.append(single_space(buf))
                buf = ""
            buf += ("" if not buf else " ") + w["text"]
            last_x = w["x"]
        if buf.strip(): cells.append(single_space(buf))
        if len(cells) >= 2: multi_count += 1
        rows.append(cells)
    if multi_count >= 2:
...
# ---------------- Core: convert a single page ----------------
def convert_page_to_docx(image, doc: Document):
    img_small = clamp_image(image)
    res = ocr.ocr(np.array(img_small))  # ✅ no cls=True
    ...
    for box, (text, conf) in lines:
        ...
        x, y, h = box_left(box), box_top(box), max(1, box_height(box))
    ...
    for col_words in columns:
        ...
        for ln in line_groups:
            ...
            if not text: continue
            left, right = min(w["x"] for w in ln), max(w["x"] + 1 for w in ln)
        ...
        for lo in line_objs:
            if not buf:
                buf.append(lo); continue
            prev = buf[-1]
            if (is_bullet(prev["text"]) or is_numbered(prev["text"])) and (is_bullet(lo["text"]) or is_numbered(lo["text"])):
                buf.append(lo)
            else:
                blocks.append(buf); buf = [lo]
        ...
        for block in blocks:
            ...
            if kind == 'heading':
                ...
                for i, item in enumerate(content, start=1):
                    p = doc.add_paragraph()
                    run = p.add_run(f"{i}. " if ordered else "• "); run.bold = True
                    p.add_run(item); p.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
            else:
                ...
                p = doc.add_paragraph()
                p.alignment = align
                r = p.add_run(content); r.font.size = size_pt
...
def pdf_to_docx():
    ...
    input_path, output_path = "/tmp/input.pdf", "/tmp/output.docx"
    f.save(input_path)
    try:
        kw = {}
    ...
    for page_idx, image in enumerate(images, start=1):
        ...
        if page_idx < len(images): doc.add_page_break()
    try:
